...
use crate::{
    ...
    parser::ast::expression::basic::variable,
...
impl Restriction {
    ...
    pub fn restriction(&self) -> &Vec<Bool> {
        &self.res
    }
...
impl Restriction {
    ...
    pub fn new_from_annotation(annotation: &NormalizedGlobalAnnotation) -> Self {
        let params = annotation
            .head()
            .variables()
            .map(|v| Int::new_const(v.name()));
        for var in annotation.head().variables() {
            Int::new_const(v)
        }
        Self { res: Vec::new() }
    }
    ...
    pub fn update_range_from_op(
        ...
        let data_value: i64 = match term {
            Operation::Primitive(Ground(ground_term)) => ground_term.value().to_i64_unchecked(),
        ...
        if let Some(restriction) = ranges.get(variable) {
            match kind {
                OperationKind::NumericGreaterthaneq => {
                    ranges.insert(
                        variable.clone(),
                        RangeRestriction::intersect_range(
                            restriction,
                            &(data_value..restriction.end),
                        ),
                    );
                }
                ...
                OperationKind::NumericLessthaneq => {
                    ranges.insert(
                        ...
                        RangeRestriction::intersect_range(
                            restriction,
                            &(restriction.start..data_value + 1),
                        ),
                    );
                }
    ...
    pub fn from_global_annotation(annotation: &NormalizedGlobalAnnotation) -> Self {
        ...
        for operation in annotation.body() {
            match operation {
                ...
                _ => panic!(
                    "Global annotations should not contain primitive operations as annotation"
                ),
            }
        }
