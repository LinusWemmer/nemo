...
impl AnnotationAnalyzer {
        ...
        for atom in rule.positive() {
            ...
            if let Some(body_restriction) = self.unary_restrictions.get(&predicate) {
                ...
                for (pos, var) in frontier_pos {
                    let restrictions_at_pos =
                        body_restriction.range_res().get(&pos).unwrap_or(&RANGE_INF);

                    let variable_restriction = variable_restrictions.get(var).unwrap_or(&RANGE_INF);

                    let updated_restriction = RangeRestriction::intersect_range(
                        restrictions_at_pos,
    ...
    pub fn verify_global_annotation(&self, annotation: &NormalizedGlobalAnnotation) -> bool {
        ...
        if let Some(propagated_restriction) = self.unary_restrictions.get(&predicate) {
            if !propagated_restriction.verify_no_empty_term() {
                panic!(
                    "There is an empty term for some annotation for {}",
                    predicate.0
                )
            }
            ...
            if !restriction_from_annotation.verify_compatibility(propagated_restriction) {
                println!(
                    "Annotation {} for {} cannot be verified",
                    restriction_from_annotation, predicate.0
                )
            } else {
                print!("{}: ", predicate.0.name());
                println!(
                    "{propagated_restriction} satisfies annotation {restriction_from_annotation}"
                );
            }
        }
        true
    ...
    pub fn propagate_annotations(&mut self) {
        ...
        let mut base_predicates: HashSet<(Tag, usize)> = self
            .program
            .predicates()
            .filter(|(pred, _)| !derived.contains(pred))
            .collect();
        ...
        for (predicate, arity) in base_predicates {
            ...
            for annotation in annotations {
                ...
                self.unary_restrictions
                    ...
                    .and_modify(|old_restriction| {
                        old_restriction.range_intersection(&new_restriction)
                    })
        ...
        while let Some(scc) = rule_graph.next_scc() {
            ...
            while delta {
                ...
                for rule_index in &scc {
                    ...
                    let mut variable_restrictions: HashMap<Variable, Range<i64>> =
                        HashMap::<Variable, Range<i64>>::default();
                    ...
                    if sat {
                        ...
                        let op_vars: HashSet<Variable> = rule
                            .operations()
                            .iter()
                            .flat_map(|op| op.variables())
                            .cloned()
                            .collect();
                        let head_vars: HashSet<Variable> = rule
                            .head()
                            .iter()
                            .flat_map(|head| head.variables())
                            .cloned()
                            .collect();
                        let arith_head_vars: HashSet<Variable> =
                            op_vars.intersection(&head_vars).cloned().collect();
                        ...
                        for head_atom in rule.head() {
                            ...
                            for (pos, var) in var_pos_in_head {
                                match (
                                    variable_restrictions.get(var),
                                    arith_head_var_ranges.get(var),
                                ) {
                                    (None, None) => (),
                                    (None, Some(range)) => {
                                    ...
                                    (Some(range_1), Some(range_2)) => {
                                        let inter_range =
                                            RangeRestriction::intersect_range(range_1, range_2);
                                        delta =
                                            head_range_res.range_union(pos, &inter_range) || delta;
                                    }
                                }
        ...
        for annotation in self.program.global_annotations() {
            if self
                .program
                .derived_predicates()
                .contains(&annotation.head().predicate())
            {
